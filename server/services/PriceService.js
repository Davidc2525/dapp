...
async function ejecutar() {
    ...
    //console.log(`1 dolar en BNB es: ${1 / bnbPrice}`);
    return bnbPrice;
}

async function ejecutar_usd() {
    let usd_price = 0;
    const uri = "https://s3.amazonaws.com/dolartoday/data.json";
    let data = {}    //console.log(data);
    try {
        data = await (await fetch(uri)).json();
        usd_price = data["USD"].promedio_real;
    } catch (error) {
        console.log(error)
...
function price_dump() {
    console.log(`
-------------------------------------------------------
        ACTUAL BNB PRICE: ${actual_price}
        ACTUAL USD PRICE: ${actual_price_usd}
        COUNT REQUEST: ${count}
-------------------------------------------------------
    `);
}
var actual_price = 0;
var actual_price_usd = 0;
var count = 0;
async function e() {
    const PORT = process.argv[2] ? process.argv[2] : 3232;
    const interval = 30;
    ...
    router.get('/usdves/:from?', async (req, res) => {
        count++;
        res.json({ actual_price: actual_price_usd, par: "USD/VES", interval, from: req.params.from, count })
    });

    router.get('/:from?', async (req, res) => {
        count++;
        res.json({
            prices: { bnb: actual_price, usd: actual_price_usd },
            interval,
            from: req.params.from, count
        })
    });
    ...
    app.listen(
        process.env.SERVICE_BNB_PRICE_PORT,
        process.env.SERVICE_BNB_PRICE_HOST,
        (error, data) => {
            if (error) {
                console.log(error)
                return false;
            ...
            console.log('runing API server at ' + process.env.SERVICE_BNB_PRICE_HOST + ':' + process.env.SERVICE_BNB_PRICE_PORT + '/bnbprice');
        });


    ejecutar().then(price => {
        if (price != 0) actual_price = parseFloat(price);
        price_dump();
    })

    ejecutar_usd().then(price => {
        if (price != 0) actual_price_usd = parseFloat(price);
        price_dump();
